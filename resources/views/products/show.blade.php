                ...
                <template x-if="canAddToCart">
                    <div class="space-y-4">
                        ...
                        <div class="flex gap-2.5">
                            @auth
                                <button @click="Livewire.dispatch('add-to-cart-qty', { productId: {{ $product->id }}, quantity: qty, variantId: selectedVariantId })"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                @foreach($product->reviews as $review)
                    <div class="p-5 bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800/60 hover:border-primary-100 dark:hover:border-primary-900/30 transition-all duration-300">
                        ...
                        @endif
                        @if($review->image)
                            <button type="button" @click="reviewImage = '{{ asset('storage/' . $review->image) }}'"
                                    class="mt-3 rounded-xl overflow-hidden shrink-0 w-20 h-20 border border-slate-100 dark:border-slate-800/60 hover:border-primary-200 dark:hover:border-primary-800 cursor-zoom-in group transition-all">
                                <img src="{{ asset('storage/' . $review->image) }}" alt="Foto ulasan" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                            </button>
                        @endif
                    </div>
                @endforeach
