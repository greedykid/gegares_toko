<div class="relative" x-data="{ open: false }" x-init="$watch('open', value => { if (value) { $wire.markAsRead() } })" @click.away="open = false">
    <button @click="open = !open" aria-label="Notifikasi" class="relative inline-flex p-2 rounded-lg text-slate-500 hover:text-primary-600 hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-all duration-200 group" title="Notifikasi">
        <svg class="w-5 h-5 transition-transform duration-200 group-hover:rotate-12" fill="none" viewBox="0 0 24 24" stroke-width="1.8" stroke="currentColor">
    ...
    <div x-show="open"
        ...
        {{-- Header --}}
        <div class="flex items-center justify-between px-5 py-4 border-b border-slate-150 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950">
        ...
        {{-- Footer --}}
        <div class="px-5 py-4 border-t border-slate-150 dark:border-slate-800/80 bg-slate-50 dark:bg-slate-950 text-center">
            <a href="{{ route('settings.index') }}#notifications" class="inline-flex items-center justify-center gap-2 text-xs font-bold text-slate-600 dark:text-slate-450 hover:text-primary-600 dark:hover:text-primary-400 transition-all duration-150">
