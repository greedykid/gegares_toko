            ...
            <table class="w-full admin-table">
                <thead>
                    <tr class="border-b border-slate-100 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-800/40">
                        ...
                        <x-admin.sort-header column="email" :sort="$sort" :dir="$dir">Email</x-admin.sort-header>
                        <x-admin.sort-header column="role" :sort="$sort" :dir="$dir" align="center">Role</x-admin.sort-header>
                        <x-admin.sort-header column="created_at" :sort="$sort" :dir="$dir" align="right">Bergabung</x-admin.sort-header>
                            ...
                            @if($user->id !== auth()->id())
                                <div class="relative group/tooltip inline-flex">
                                    <form method="POST" action="{{ route('admin.users.destroy', $user) }}" class="inline" onsubmit="return confirm('Hapus pengguna ini?')">
                                        @csrf @method('DELETE')
                                        <button type="submit" class="p-2 text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/60 rounded-xl transition-all" title="Hapus">
