            ...
            <div class="flex items-center gap-1 overflow-x-auto scrollbar-none -mx-1 px-1">
                @foreach($stockTabs as $val => $label)
                    <a href="{{ request()->fullUrlWithQuery(['stock_status' => $val, 'page' => null]) }}"
                       class="shrink-0 px-3.5 py-1.5 rounded-lg text-sm font-semibold transition-colors {{ (string) $stockTab === (string) $val ? 'bg-primary-600 text-white' : 'text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800' }}">
                        {{ $label }}
                    </a>
                @endforeach
            </div>
