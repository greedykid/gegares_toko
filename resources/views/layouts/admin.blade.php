...
<body class="bg-slate-50 dark:bg-slate-950 font-sans antialiased transition-colors duration-300" 
      x-data="{ 
        sidebarOpen: true, 
        sidebarMobile: false,
    ...
    @php
        $menuGroups = [
            ...
            'Sistem & Keamanan' => [
                ['route' => 'admin.security.index', 'label' => 'Keamanan AI', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75m-3-7.036A11.959 11.959 0 0 1 3.598 6 11.99 11.99 0 0 0 3 9.75c0 5.592 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.57-.598-3.75h-.152c-3.196 0-6.1-1.249-8.25-3.286Zm0 13.036h.008v.008H12v-.008Z"/>'],
                ['route' => 'admin.activity-logs.index', 'label' => 'Activity Log', 'icon' => '<path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m3.75 9v6m3-3H9m1.5-12H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />'],
    ...
    <div class="flex min-h-screen">
        ...
        <div x-show="sidebarMobile" class="fixed inset-0 z-40 lg:hidden" style="display:none;">
            ...
            <div x-show="sidebarMobile"
                 ...
                 class="fixed inset-0 bg-slate-900/50" @click="sidebarMobile = false"></div>
            
            <aside x-show="sidebarMobile" 
                ...
                <div class="flex items-center justify-between px-6 h-16 border-b border-slate-100 shrink-0">
                    <span class="text-xl font-extrabold text-primary-700">gegares</span>
                    <button @click="sidebarMobile = false" class="p-2 -mr-2 rounded-lg text-slate-400 hover:text-slate-600">
                ...
                <nav class="flex-1 py-4 px-3 space-y-6 overflow-y-auto">
                    @foreach($menuGroups as $groupLabel => $items)
                        <div class="space-y-1">
                            <h3 class="px-3 text-[10px] font-bold uppercase tracking-widest text-slate-400 dark:text-slate-500 mb-2">{{ $groupLabel }}</h3>
                            <div class="space-y-1">
                ...
                <div class="p-4 border-t border-slate-100 dark:border-slate-800">
                    <a href="{{ route('home') }}" class="flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50/50 dark:hover:bg-primary-900/20 transition-all">
                        <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"/></svg>
        ...
        {{-- Desktop Sidebar --}}
        <aside class="hidden lg:flex flex-col bg-white dark:bg-slate-900 border-r border-slate-200/60 dark:border-slate-800/60 sticky top-0 h-screen transition-all duration-300 ease-in-out shrink-0"
               :class="sidebarOpen ? 'w-64' : 'w-20'">
            {{-- Logo --}}
            <div class="flex items-center justify-between px-6 h-16 border-b border-slate-100 dark:border-slate-800">
                <a href="{{ route('admin.dashboard') }}" class="flex items-center gap-2">
                    <span class="text-xl font-extrabold text-primary-700" x-show="sidebarOpen" x-transition>gegares</span>
                    <span class="text-xl font-extrabold text-primary-700" x-show="!sidebarOpen" x-transition>G</span>
                </a>
                <button @click="sidebarOpen = !sidebarOpen" class="p-1.5 rounded-lg text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-all">
            ...
            {{-- Menu --}}
            <nav class="flex-1 overflow-y-auto py-4 px-3 space-y-6 scrollbar-thin scrollbar-thumb-slate-200 dark:scrollbar-thumb-slate-800">
                @foreach($menuGroups as $groupLabel => $items)
                    <div class="space-y-1">
            ...
            <div class="p-3 border-t border-slate-100 dark:border-slate-800">
                <a href="{{ route('home') }}" class="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-500 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50/50 dark:hover:bg-primary-900/20 transition-all">
                    <svg class="w-5 h-5 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M13.5 6H5.25A2.25 2.25 0 0 0 3 8.25v10.5A2.25 2.25 0 0 0 5.25 21h10.5A2.25 2.25 0 0 0 18 18.75V10.5m-10.5 6L21 3m0 0h-5.25M21 3v5.25"/></svg>
        ...
        <div class="flex-1 flex flex-col min-w-0">
            {{-- Top Navbar --}}
            <header class="sticky top-0 z-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-lg border-b border-slate-200/60 dark:border-slate-800/60 h-16 flex items-center px-6 transition-colors duration-300">
                <button @click="sidebarMobile = !sidebarMobile" class="lg:hidden p-2 rounded-lg text-slate-500 hover:bg-slate-100 mr-3">
                    <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5"/></svg>
                </button>
                <h2 class="text-lg font-bold text-slate-900 dark:text-slate-100">@yield('page_title', 'Dashboard')</h2>
                <div class="ml-auto flex items-center gap-3">
                    {{-- Theme Toggle --}}
                    <button @click="toggleTheme()" class="p-2 rounded-lg text-slate-500 dark:text-slate-400 hover:text-primary-600 dark:hover:text-primary-400 hover:bg-primary-50 dark:hover:bg-primary-900/20 transition-all duration-200" title="Ganti Tema">
                        {{-- Light --}}
                        ...
                        {{-- Dark --}}
                        <svg x-show="theme === 'dark'" class="w-5 h-5" x-cloak fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M21.752 15.002A9.718 9.718 0 0 1 18 15.75c-5.385 0-9.75-4.365-9.75-9.75 0-1.33.266-2.597.748-3.752A9.753 9.753 0 0 0 3 11.25C3 16.635 7.365 21 12.75 21a9.753 9.753 0 0 0 9.002-5.998z"/></svg>
                        {{-- System --}}
                        <svg x-show="theme === 'system'" class="w-5 h-5" x-cloak fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 17.25v1.007a3 3 0 0 1-.879 2.122L7.5 21h9l-.621-.621A3 3 0 0 1 15 18.257V17.25m6-12V15a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 15V5.25m18 0A2.25 2.25 0 0 0 18.75 3H5.25A2.25 2.25 0 0 0 3 5.25m18 0V12a2.25 2.25 0 0 1-2.25 2.25H5.25A2.25 2.25 0 0 1 3 12V5.25"/></svg>
                    </button>
                    {{-- Profile --}}
                    <div class="relative" x-data="{ open: false }">
                        <button @click="open = !open" class="flex items-center gap-2 px-3 py-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors duration-200">
                            <div class="w-8 h-8 rounded-lg bg-linear-to-br from-primary-400 to-primary-600 flex items-center justify-center text-white text-sm font-semibold shadow-sm">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</div>
                        ...
                        <div x-show="open" @click.away="open = false" 
                             ...
                             class="absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-lg ring-1 ring-slate-200/60 dark:ring-slate-800 py-1.5 z-50 overflow-hidden" style="display:none;">
                            <a href="{{ route('home') }}" class="flex items-center gap-2 px-4 py-2.5 text-sm text-slate-700 dark:text-slate-300 hover:bg-primary-50 dark:hover:bg-primary-900/20 hover:text-primary-700 dark:hover:text-primary-400 transition-colors">
                            ...
                            <form method="POST" action="{{ route('admin.logout') }}">
                                @csrf
                                <button type="submit" class="flex items-center gap-2 w-full text-left px-4 py-2.5 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/40 transition-colors">
    ...
    {{-- Toast Manager --}}
    <div x-data="toastManager()" @toast.window="addToast($event.detail)" class="fixed top-20 right-4 z-50 flex flex-col gap-3 w-80">
