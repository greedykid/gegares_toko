...
@section('content')
    ...
    <div>
        <h1 class="text-3xl font-black text-slate-900 dark:text-white tracking-tight leading-none mb-2">Selamat Datang</h1>
        <p class="text-sm text-slate-500 dark:text-slate-400 font-medium mb-8">Silakan masuk ke akun untuk melanjutkan
            belanja.</p>

        @if(session('info') || session()->has('checkout.pending'))
            <div class="mb-6 p-4 rounded-xl bg-primary-50 dark:bg-primary-950/30 border border-primary-200 dark:border-primary-900/50 flex items-start gap-3">
                ...
                <p class="text-sm font-semibold text-primary-800 dark:text-primary-300">{{ session('info') ?? 'Masuk atau daftar untuk menyelesaikan pesanan Anda.' }}</p>
            </div>
        @endif
        ...
        <form id="login-form" x-data="{ loading: false }" @submit="loading = true" method="POST" action="{{ route('login') }}" class="space-y-5">
            @csrf
            <input type="hidden" name="g-recaptcha-response" id="g-recaptcha-response">
            <div>
                <label for="email"
                    class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5 ml-0.5">Email</label>
                ...
                <label class="group flex items-center gap-3 cursor-pointer select-none">
                    <div class="relative flex items-center">
                        <input type="checkbox" name="remember"
                            class="peer h-5 w-5 cursor-pointer appearance-none rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/50 checked:bg-primary-600 checked:border-primary-600 transition-all duration-200">
                    ...
                    <span
                        class="text-sm font-semibold text-slate-500 dark:text-slate-400 group-hover:text-primary-600 dark:group-hover:text-primary-400 transition-colors">Ingat
                        saya</span>
                </label>
            </div>
            <button type="submit"
            ...
            <div class="relative py-4">
                ...
                <div class="relative flex justify-center text-xs uppercase">
                    <span
                        class="bg-slate-50 dark:bg-slate-950 px-3.5 text-slate-400 dark:text-slate-500 font-black tracking-widest transition-colors">Atau</span>
                </div>
            </div>
            ...
            <a href="{{ route('auth.google') }}"
                ...
                <svg class="w-5 h-5 shrink-0" viewBox="0 0 24 24">
                    <path fill="#ea4335"
                        d="M5.2662 9.76451C6.1982 6.95542 8.8542 4.90909 12 4.90909C13.6909 4.90909 15.2182 5.50909 16.4182 6.49091L19.9091 3C17.7818 1.14545 15.0545 0 12 0C7.27273 0 3.19091 2.69091 1.24545 6.65455L5.2662 9.76451Z" />
                    <path fill="#34a853"
                        d="M16.0409 18.0136C14.8705 18.7159 13.4841 19.0909 12 19.0909C8.8542 19.0909 6.1982 17.0455 5.2662 14.2364L1.24545 17.3455C3.19091 21.3091 7.27273 24 12 24C15.0545 24 17.7818 23.0182 19.9318 21.3273L16.0409 18.0136Z" />
                    <path fill="#4285f4"
                        d="M23.64 12.2182C23.64 11.3364 23.5545 10.5182 23.3909 9.70909H12V14.2473H18.5182C18.2364 15.7455 17.3909 17.0273 16.1261 17.8909L20.0171 21.2045C22.28 19.1182 23.64 16.0091 23.64 12.2182Z" />
                    <path fill="#fbbc05"
                        d="M5.2662 14.2364C5.0182 13.5273 4.8818 12.7727 4.8818 12C4.8818 11.2273 5.0182 10.4727 5.2662 9.76364L1.24545 6.65455C0.445455 8.27273 0 10.0818 0 12C0 13.9182 0.445455 15.7273 1.24545 17.3455L5.2662 14.2364Z" />
                </svg>
...
@push('scripts')
    ...
    @else
        <script>
            // Bypass reCAPTCHA on local environment to prevent API 401 unauthorized blocking
            document.getElementById('login-form').addEventListener('submit', function (e) {
