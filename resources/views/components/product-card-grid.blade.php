...
<div class="card-rise @container group relative flex flex-col bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800/60 hover:border-primary-200 dark:hover:border-primary-700/60 shadow-sm hover:shadow-xl hover:shadow-primary-200/40 dark:hover:shadow-black/40 hover:-translate-y-1.5 transition-all duration-500 ease-out overflow-hidden {{ $product->isOutOfStock() ? 'product-out-of-stock' : '' }}">
    ...
    <div class="flex flex-col flex-1 p-3 sm:p-4">
        ...
        <div class="mt-3">
            ...
            @else
                {{-- Add-to-cart is open to guests too: the cart lives in the session,
                     login is only required at the final checkout step. --}}
                <button
                    ...
                    {{-- Label depends on the CARD width (container query), not the
                         viewport: at 4 columns the card is too narrow for the full
                         phrase, so the suffix is dropped until the card has room. --}}
                    <span x-show="loading" style="display:none">Memproses...</span>
                    <span x-show="!loading">Tambah<span class="hidden @min-[232px]:inline"> ke Keranjang</span></span>
                </button>
            @endif
        </div>
    </div>
